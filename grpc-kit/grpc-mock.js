...
createMockServer = ({ rules, ...config }) => {
  const routesFactory = rules.reduce(
    (_routesFactory, { method, streamType, stream, input, output, error }) => {
      const handlerFactory =
        _routesFactory.getHandlerFactory(method) ||
        _routesFactory.initHandlerFactory(method);
      handlerFactory.addRule({
        method,
        streamType,
        stream,
        input,
  ...
  grpcServer.clearInteractions = () =>
    Object.keys(routes).forEach(
      method => (routes[method].interactions.length = 0)
    );
...
class RoutesFactory {
  ...
  generateRoutes() {
    return Object.entries(this.routebook).reduce(
      (_routes, [method, handlerFactory]) => {
...
const prepareMetadata = error => {
  ...
  if (error.metadata) {
    const grpcMetadata = Object.entries(error.metadata).reduce(
      (m, [k, v]) => (m.add(k, String(v)), m),
      new Metadata()
    );
    errorFields = [...errorFields, ["metadata", grpcMetadata]];
  }
          ...
          call.on(
            ...
            function(stream, memo, data) {
              ...
              if (error) {
                ...
                call.emit(
                  "error",
                  prepareMetadata(UNEXPECTED_INPUT_PATTERN_ERROR)
                );
                call.end();
              }
